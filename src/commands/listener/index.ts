...
export default class Listener extends Command {
  ...
  async run(): Promise<void> {
    ...
    const processTransactions = (network: string, transactions: any, callback: any) => {
    ...
    const processBlock = async (job: any) => {
    ...
    function handleRinkebyDroppedSocket(error: Error) {
      ...
      rinkebyResetProvider = setInterval(() => {
        ...
        try {
          ...
          providers.rinkeby.on('error', handleRinkebyDroppedSocket)
          providers.rinkeby.on('close', handleRinkebyDroppedSocket)
          providers.rinkeby.on('end', handleRinkebyDroppedSocket)
    ...
    function handleMumbaiDroppedSocket(error: Error) {
      ...
      mumbaiResetProvider = setInterval(() => {
        ...
        try {
          ...
          providers.mumbai.on('error', handleMumbaiDroppedSocket)
          providers.mumbai.on('close', handleMumbaiDroppedSocket)
          providers.mumbai.on('end', handleMumbaiDroppedSocket)
