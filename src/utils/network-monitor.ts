...
export class NetworkMonitor {
  ...
  static transferEventFragment: ethers.utils.EventFragment = ethers.utils.EventFragment.from('Transfer(address indexed _from, address indexed _to, uint256 indexed _tokenId)')
  static availableJobEventFragment: ethers.utils.EventFragment = ethers.utils.EventFragment.from('AvailableJob(bytes payload)')
  static bridgeableContractDeployedEventFragment: ethers.utils.EventFragment = ethers.utils.EventFragment.from('BridgeableContractDeployed(address indexed contractAddress, bytes32 indexed hash)')
    ...
    if ('logs' in receipt && receipt.logs !== null && receipt.logs.length > 0) {
      for (let i = 0, l = receipt.logs.length; i < l; i++) {
        ...
        if (log.topics[0] === this.targetEvents.Packet) {
          const packetPayload = NetworkMonitor.iface.decodeEventLog(NetworkMonitor.packetEventFragment, log.data, log.topics)[1] as string
    ...
    if ('logs' in receipt && receipt.logs !== null && receipt.logs.length > 0) {
      for (let i = 0, l = receipt.logs.length; i < l; i++) {
        ...
        if (log.topics[0] === this.targetEvents.Transfer) {
          const event: string[] = NetworkMonitor.iface.decodeEventLog(NetworkMonitor.transferEventFragment, log.data, log.topics) as string[]
    ...
    if ('logs' in receipt && receipt.logs !== null && receipt.logs.length > 0) {
      for (let i = 0, l = receipt.logs.length; i < l; i++) {
        ...
        if (log.address.toLowerCase() === this.operatorAddress && log.topics[0] === this.targetEvents.AvailableJob) {
          return NetworkMonitor.iface.decodeEventLog(NetworkMonitor.availableJobEventFragment, log.data, log.topics)[0] as string
        }
      }
    }
  ...
  decodeBridgeableContractDeployedEvent(receipt: ethers.ContractReceipt): string[] | undefined {
    if ('logs' in receipt && receipt.logs !== null && receipt.logs.length > 0) {
      for (let i = 0, l = receipt.logs.length; i < l; i++) {
        const log = receipt.logs[i]
        if (log.address.toLowerCase() === this.factoryAddress && log.topics[0] === this.targetEvents.BridgeableContractDeployed) {
          return NetworkMonitor.iface.decodeEventLog(NetworkMonitor.bridgeableContractDeployedEventFragment, log.data, log.topics) as string[]
        }
      }
    }
